...
import ProjectUserModal from "../../EmployeesProfile/UserProjects/components/ProjectUserModal";
import { postNewProjectForUser } from "../../../Components/db/Redux/api/ProjectDetailSlice";

const ModalComponent = ({ open, handleClose, userData }) => {
  ...
  const [endDateProject, setEndDateProject] = useState(dayjs);
  const [startDateProject, setStartDateProject] = useState(dayjs);
  ...
  const handleSubmit = () => {
    ...
    if (
      ...
      dispatch(postNewProjectForUser(data));
      handleClose();
      setUsers([]);
      setWorkers([]);
      setValue("");
